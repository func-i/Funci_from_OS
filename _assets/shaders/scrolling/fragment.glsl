...
vec2 calculate_rot_values(vec2 uv) {
  ...
  float noise = 0.5 * is_high_enough * modulation * (snoise(vec3(2.5 * get_uv_coords() + time, time) - 0.5)) ;
  return vec2(noise * 0.3 + texture_value  + (1./256.), noise * 0.4 + 0.5);
}

void main()	{
  vec2 rotation_values = calculate_rot_values(get_uv_coords());
	gl_FragColor = vec4(rotation_values, rotation_values);
}
